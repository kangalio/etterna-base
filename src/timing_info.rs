...
impl TimingInfo {
	pub fn from_sm_bpm_string(string: &[u8]) -> Result<Self, SmBpmStringParseError> {
		...
		for pair in string.split(|&c| c == b',') {
			let equal_sign_index = pair
				.iter()
				.position(|&c| c == b'=')
				.ok_or(SmBpmStringParseError::MissingEquals)?;
			let beat: f64 =
				lexical_core::parse_lossy(crate::util::trim_bstr(&pair[..equal_sign_index]))
					.map_err(SmBpmStringParseError::InvalidBeatString)?;
			let bpm: f64 =
				lexical_core::parse_lossy(crate::util::trim_bstr(&pair[equal_sign_index + 1..]))
		...
		if changes[0].beat != 0.0 {
			return Err(SmBpmStringParseError::FirstBpmChangeNotZero {
				beat_instead: changes[0].beat,
			});
		}
	...
	pub fn ticks_to_seconds(&self, ticks: &[u32]) -> Vec<f32> {
		...
		let mut convert_ticks_up_to =
			|beat: f64, cursor_second: f64, cursor_beat: f64, beat_time: f64| {
